...
function TypeWriter() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (!del) {
        if (sub < cur.length) setSub(s => s+1)
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', justifyContent:'center', padding:'52px 40px 60px' }} className="page-pad">
      ...
      <div className="reveal" style={{ marginBottom:'28px' }}>
        <span className="badge-pulse" style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'rgba(0,200,100,.1)', border:'1px solid rgba(0,200,100,.3)', borderRadius:'9999px', padding:'6px 16px', fontSize:'11px', fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase', color:'rgba(0,230,110,.95)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'48px', alignItems:'center', marginBottom:'52px' }}>
        <div>
          {/* Name */}
          <h1 className="reveal reveal-delay-1" style={{ fontSize:'clamp(52px,8vw,96px)', fontWeight:900, letterSpacing:'-.04em', lineHeight:.95, marginBottom:'20px' }}>
          ...
          {/* Typewriter */}
          <div className="reveal reveal-delay-2" style={{ fontSize:'clamp(16px,2.2vw,22px)', marginBottom:'24px', height:'32px' }}>
            <TypeWriter/>
          </div>

          {/* Bio */}
          <p className="reveal reveal-delay-3" style={{ fontSize:'15px', color:'rgba(255,255,255,.55)', lineHeight:1.85, maxWidth:'520px', marginBottom:'32px' }}>
          ...
          <p style={{ fontSize:'12px', color:'rgba(255,255,255,.3)', marginBottom:'28px' }} className="reveal reveal-delay-3">
          ...
          <div className="reveal reveal-delay-4" style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
            {[
              ...
              <a key={btn.label} href={btn.href} target="_blank" rel="noreferrer"
                ...
                onMouseEnter={e=>{ e.currentTarget.style.transform='translateY(-3px)'; e.currentTarget.style.boxShadow='0 12px 32px rgba(124,122,207,.3)' }}
      ...
      {/* Stats */}
      <div className="reveal reveal-delay-5" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'14px', maxWidth:'720px' }}>
        {STATS.map(s => (
          <div key={s.val} className="stat-float card-3d" style={{ background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)', borderRadius:'18px', padding:'18px 16px', textAlign:'center', backdropFilter:'blur(12px)' }}>
